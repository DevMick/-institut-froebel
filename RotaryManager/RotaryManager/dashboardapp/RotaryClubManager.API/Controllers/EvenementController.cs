...
namespace RotaryClubManager.API.Controllers
{
    ...
    public class EvenementController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateEvenement(Guid clubId, Guid id, [FromBody] UpdateEvenementRequest request)
        {
            try
            {
                _logger.LogInformation("Mise à jour de l'événement - Début de la requête");
                _logger.LogInformation("ClubId: {ClubId}, EventId: {EventId}", clubId, id);
                _logger.LogInformation("Données reçues: {@Request}", request);
                _logger.LogInformation("Headers: {@Headers}", Request.Headers);

                // Validation des paramètres
                if (clubId == Guid.Empty)
                {
                    _logger.LogWarning("ClubId invalide");
                    return BadRequest("L'identifiant du club est invalide");
                }

                if (id == Guid.Empty)
                {
                    _logger.LogWarning("EventId invalide");
                    return BadRequest("L'identifiant de l'événement est invalide");
                }
                ...
                if (!await CanManageClub(clubId))
                {
                    _logger.LogWarning("Accès non autorisé pour le club {ClubId}", clubId);
                    return Forbid("Vous n'avez pas l'autorisation de gérer ce club");
                }
                ...
                {
                    _logger.LogWarning("Événement non trouvé - ClubId: {ClubId}, EventId: {EventId}", clubId, id);
                    return NotFound(new { message = $"Événement avec l'ID {id} non trouvé dans le club {clubId}" });
                }

                _logger.LogInformation("Valeur actuelle de EstInterne: {EstInterne}", existingEvenement.EstInterne);
                _logger.LogInformation("Nouvelle valeur de EstInterne: {EstInterne}", request.EstInterne);
                ...
                _logger.LogInformation("Données avant sauvegarde: {@Evenement}", existingEvenement);

                await _context.SaveChangesAsync();

                _logger.LogInformation("Événement {Id} mis à jour avec succès dans le club {ClubId}", id, clubId);

                return NoContent();
            }
            catch (DbUpdateConcurrencyException)
            {
                _logger.LogError("Erreur de concurrence lors de la mise à jour de l'événement {Id}", id);
